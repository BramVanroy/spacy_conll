...
def init_parser(parser: str = 'spacy',
    ...
    :param pipeline_opts: will be passed to the core pipeline. For spacy and udpipe, it will be passed to their
    ...
    """
    pipeline_opts = {} if pipeline_opts is None else pipeline_opts

    if parser == 'spacy':
        nlp = spacy.load(model_or_lang, **pipeline_opts)
        if is_tokenized:
        ...
        snlp = stanfordnlp.Pipeline(lang=model_or_lang, tokenize_pretokenized=is_tokenized, **pipeline_opts)
        nlp = StanfordNLPLanguage(snlp)
    elif parser == 'stanza':
        import stanza
        from spacy_stanza import StanzaLanguage

        snlp = stanza.Pipeline(lang=model_or_lang, tokenize_pretokenized=is_tokenized, **pipeline_opts)
        nlp = StanzaLanguage(snlp)
    elif parser == 'udpipe':
        import spacy_udpipe

        nlp = spacy_udpipe.load(model_or_lang, **pipeline_opts)
    else:
